...
def analyze_resume_with_llm(resume_text, job_role):
    ...
    try:
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "You are a professional ATS resume optimization assistant. You MUST output EXACTLY 5 sections, and each MUST begin with a Markdown H2 header: '## Strengths', '## Flaws & Weaknesses', '## Missing Elements', '## Actionable Additions', and '## ATS Optimization Tips'. Do not use numbered lists for the section headers. Under each section header, provide 3 to 4 concise bullet points. Provide direct, actionable feedback without any introduction, conclusion, or pleasantries."},
                {"role": "user", "content": f"Analyze this resume for the job role: {job_role}. Output the 5 sections using ## headers as instructed.\n\nResume content:\n{truncated_resume}"}
            ],
            "max_tokens": 1000,
            "temperature": 0.7
        }
